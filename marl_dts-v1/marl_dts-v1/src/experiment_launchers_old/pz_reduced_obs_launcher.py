...
def compute_features(obs):
    ...
    n_teammates_left = np.sum(nondead[:, :map_w // 2])
    n_teammates_up = np.sum(nondead[:map_h // 2, :])
    n_teammates_right = np.sum(nondead[:, 1+map_w // 2:])
    n_teammates_down = np.sum(nondead[1+map_h // 2:, :])
        ...
        for index, agent_name in enumerate(env.agent_iter()):
            ...
            if not dones[i]:
                if agents[agent_name] is not None:
                    ...
                    env.step(agents[agent_name].get_output(compute_features(obs)))
                else:
