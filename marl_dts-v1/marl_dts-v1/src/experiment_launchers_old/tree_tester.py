...
def attention(observation):
    ...
    return np.array([paddle_pos2[1]/160,ball_pos2[0]/160,ball_pos2[1]/160,direction[0],direction[1], 0.5])
...
if __name__ == "__main__":
    ...
    states = [0,1]
    actions = [2,3]  
    mapper = Qlearning(states, actions)
    episodes = 10
    ...
    for e in range(episodes):
        ...
        while not done:
            obs = attention(obs)
            state = tree_out(obs)
            action = mapper.choose_action(state)
            obs, rew, done, _ = env.step(action)
            mapper.set_reward(rew)
