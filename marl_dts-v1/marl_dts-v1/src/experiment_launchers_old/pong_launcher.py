...
def evaluate(pairs, config):
    ...
    for i in range(config["training"]["episodes"]):
        ...
        while not done:
            obs = attention(obs, queries, config)
...
def mix(sequence, config):
    ...
    while not all_elements_contained:
        ...
        mixed_indices = np.random.randint(0, len(sequence), seq_len)
...
def produce_tree(config, log_path):
    ...
    # Automatically compute input indices
    # NOTE: If you modify the features computed, you have to modify the
    #       * 2 coefficient
    config["grammar"]["input_index"] = {
        "start": 0,
        "stop": config["attention"]["n_queries"] * 2,
        "step": 1,
        "dtype": "int"
    }
    ...
    for i in range(config["training"]["generations"]):
        ...
        trees = map_(utils.genotype2phenotype, tpop, config)
